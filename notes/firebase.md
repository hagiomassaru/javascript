...
```
Somente se estiver logado (qualquer usuario):
...
Somente o usuario com o **id** `KMbmGBRYhkQNWHeLBvtP3OA4RUT2` podem escrever:
...
service cloud.firestore {
  match /databases/{database}/documents {
    match /{document=**} {
      allow read: if true;
      allow write: if request.auth.uid == "KMbmGBRYhkQNWHeLBvtP3OA4RUT2" ;
    }
  }
}
```
